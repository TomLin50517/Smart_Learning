 ...
 *
 * 已實作（Phase 2-4，SD §6.14）：我的證書、課程證書清單、撤銷、公開驗證；PDF 下載（SD §6.28）。
 * 發證由 worker 的 certificate.generate 執行（選課完成時由學習模組在同一交易排入），PDF 也在該處產生。
 * LearningRecordModule：撤銷時寫入 certificate.revoked 學習事件（LEARNING_EVENTS）。KnowledgeModule：取得 OBJECT_STORAGE。
 */
@Module({
  imports: [LearningRecordModule, KnowledgeModule],
