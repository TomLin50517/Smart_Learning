...
const HEADERS: Record<'email' | 'displayName' | 'role' | 'courseCode', string[]> = {
...
function headerIndex(row: string[]): Partial<Record<keyof typeof HEADERS, number>> | null {
...
export function csvTemplate(kind: 'members' | 'learners'): string {
  const lines =
    kind === 'members'
      ? ['Email,姓名,角色,課程代碼', 'student01@example.com,王小明,學員,C-0001', 'student02@example.com,李小華,學員,', 'teacher01@example.com,陳老師,講師,C-0001']
