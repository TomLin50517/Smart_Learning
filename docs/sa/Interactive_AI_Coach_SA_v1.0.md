...
| v1.15 | 2026-09-13 | UC-CRS-007／008 實作：SEQ-01 五項檢查的具體判定（C1 以先修依賴圖＋導覽模式判斷無法到達；發布前必須設定完成條件；C5 以元件 schema 驗證設定與答案），發布於同一交易內重跑檢查並寫入內容快照雜湊（SD §6.7） | System Analyst |
...
| v1.23 | 2026-09-14 | 教材上傳與解析（Phase 3-1，UC-KNW-001～003、SEQ-06）：課程人員於草稿版本上傳 PDF／Word（.docx）／Markdown／純文字；格式以檔頭判斷；背景擷取文字並切段；課程人員可預覽擷取文字、重試失敗、上傳新版（已發布版本仍引用舊版）、移出或刪除（已發布版本使用中不可刪）。學員不直接瀏覽教材，只經 AI 教練的引用開啟（Phase 3-4）。**惡意程式掃描 hook 與掃描影像 PDF 的文字辨識（OCR）尚未提供**（SD §6.17） | System Analyst |
| v1.24 | 2026-09-14 | 教材檢索（Phase 3-2）：教材處理完成後建立全文索引（中文以相鄰兩字比對，目前不含語意檢索）；檢索一律限定組織、課程版本、驗證狀態與可見範圍（INV-7、INV-T6）；綁定改變後索引自動跟上；上傳新版後舊版轉「已被取代」但已發布版本仍可引用；課程人員可「測試搜尋」確認學員的問題會找到哪些段落（SD §6.18） | System Analyst |
| v1.25 | 2026-09-14 | AI 教練問答（Phase 3-3，UC-CCH-001～003、SEQ-07）：供應商為 Claude（預設）或 OpenAI 相容服務，金鑰由部署者放在環境設定；只送問題、教材摘錄與學習情境摘要（不送姓名、email、學號）；回答須引用本課程教材並經驗證，不通過時修正一次或改為固定的安全回答；教練不評分、不改成績（宣稱改分即攔下）；組織可停用教練並設定逐字稿可見性；每日 token 額度；教師可在任一版本測試。**未設定 AI 服務時教練顯示暫時無法使用，其餘功能正常**（SD §6.19） | System Analyst |
| v1.26 | 2026-09-14 | AI 教練介面（Phase 3-4）：學員在學習頁「問教練」，先看到找到的教材、回答附可點的出處並能開啟原文；教師在課程版本編輯頁測試教練；組織管理員在「AI 教練設定」停用教練、設定逐字稿可見性並查看今日用量（SD §6.20） | System Analyst |
| v1.27 | 2026-09-14 | AI 教練補完（UC-CCH-002、ADR-028）：學員可在作答結果旁請教練說明（只送結果的狀態、分數與問題代碼，不送作答內容；教練不改結果）；課程人員看匿名使用統計（使用學員未達匿名門檻不顯示，各活動亦同）；組織開放且對話開始時也開放時，課程人員才能閱讀逐字稿，每次閱讀都留稽核且學員看得到（SD §6.21） | System Analyst |
| v1.28 | 2026-09-14 | 組織 AI 金鑰（需求方：AI 經 LiteLLM gateway、每個組織一把虛擬金鑰，發放與備援由客戶的系統負責）：平台管理員為各組織設定金鑰與代號並可測試連線；金鑰加密存放、只能寫入不能讀出，組織管理員只看得到代號；沒有金鑰的組織不能使用 AI 教練——學員畫面直接隱藏，其他教學功能不受影響；額度用完時顯示「AI 教練休息中」（SD §6.22、ADR-034） | System Analyst |
| v1.29 | 2026-09-15 | 課程素材：老師在課程的素材庫上傳圖片（PNG／JPG／WebP）與影片（MP4／WebM），在課節中放圖片（須填替代文字）與影片，影片活動也可直接用素材庫的影片並追蹤觀看比例；素材只有課程人員與這門課的學員看得到；被課程版本使用的素材不能刪除（SD §6.23） | System Analyst |
...
| v1.34 | 2026-09-16 | 維運（UC-CRT-002、UC-PLT-008／010）：證書改為發證時產生 PDF（含查驗 QR code），學員與課程人員可直接下載，網頁列印仍保留；每日自動備份資料庫與檔案（保留 7 日／4 週／6 月），平台管理員可手動觸發並在「系統狀態」頁看到結果，還原維持人工步驟；新增系統狀態頁（背景工作、教材處理、AI 用量、儲存、授權、備份與告警）與維運指標、Prometheus 告警規則（SD §6.28） | System Analyst |
| v1.35 | 2026-09-16 | 上傳安全（UC-KNW-001、SEQ-06）：教材上傳後在移出隔離區之前掃描惡意程式，掃到就退件並在教材列表標明原因；掃描服務暫時不可用時教材留在「處理中」等待重試，不會放行未經掃描的檔案；Word 檔另防 zip bomb（解壓後過大或壓縮比異常即退件）。沒有部署掃描服務的環境照常運作，但系統會在啟動時記錄「未啟用掃描」（SD §6.29） | System Analyst |
| v1.36 | 2026-09-16 | 掃描版教材的文字辨識（UC-KNW-002）：整份掃描成圖片、沒有文字層的 PDF 過去一律以「沒有文字」退件，現在會自動辨識其中的文字（繁體中文與英文），辨識結果如同一般教材可供 AI 教練引用；辨識不出內容時仍以「沒有文字」退件。預設只處理前 50 頁（辨識很耗時）。未部署辨識工具的環境行為不變（SD §6.30） | System Analyst |
| v1.37 | 2026-09-17 | 語意檢索（UC-KNW-002、SEQ-07）：AI 教練除了關鍵字比對，也能找到「意思相近但用詞不同」的教材段落與常見問答，學員換個說法問也找得到內容；未設定向量服務或該服務暫時故障時，自動退回原本的關鍵字檢索，教練仍可正常回答。啟用後每段教材原文會送到設定的向量服務產生索引（SD §6.31） | System Analyst |
| v1.38 | 2026-09-17 | 公開首頁（UC-CMS-001～005）：平台與各組織都能以區塊（主視覺、文字、公告、提示、頁尾）編排自己的對外首頁，草稿存檔不影響線上內容，按下發布才更新；可查看發布紀錄並回到任何一個舊版本（回滾會成為新的一版，歷史不被改寫）。首頁不接受 HTML 或非 http(s) 連結，圖片影片只能引用素材庫；未發布的內容與未被引用的素材都不會對外顯示（SD §6.32） | System Analyst |
