...
## 6.10 學習畫面實作（Phase 2-2b，v1.20）

實作：`apps/web/src/pages/LearnPage.tsx`、`activity-panel.tsx`、`learn-lib.ts`（純邏輯，有單元測試）。只用 2-2a 的 API，後端不變。

| 項目 | 實作 |
|---|---|
| 路由 | `/app/learn`（我的課程）→ `/app/learn/:enrollmentId`（課程播放頁，`?lesson=` 記住目前課節）。可學習的選課顯示「進入課程」，已完成顯示「回顧課程」（唯讀：可看內容與成績、不能作答） |
| 版面 | 上方進度（必修完成數、目前總分、未完成原因的中文說明）；左側大綱（單元／課節，每個活動以符號表示狀態，另附 aria-label 文字）；右側課節內容。預設打開第一個還有事可做的課節 |
| 內容區塊 | 文字：`parseMarkdownLite` 只認標題、清單、段落，輸出純文字結構由 React 轉義——不產生、不插入 HTML（§7.5 allowlist 的最小實作）。提示框照樣式顯示。「放置活動」區塊在該處嵌入活動；沒被放置的活動列在課節最後。圖片／影片區塊待素材管理 |
| 活動面板 | 狀態、最佳成績、次數；鎖定時說明原因（學習順序／先修條件）；「開始」＝取得 runtime 後沿用進行中的作答或建立新作答；送出後顯示結果，未達上限可「再做一次」。422 時保留作答讓學員修正後重送 |
